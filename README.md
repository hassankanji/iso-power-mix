...
Known simplifications: NYISO "Dual Fuel" units (oil-or-gas capable) count as gas, so severe winter oil-switching days overstate NYISO gas somewhat; SPP's day boundary is GMT (a few hours off the others); MISO's most recent day can briefly show hydro folded into "other" until its detailed feed catches up next morning.

**Storage (batteries):** shown as *net* — energy discharged minus energy consumed charging — so an average day is slightly negative (batteries are net consumers after round-trip losses). This is the correct convention for a generation stack: counting discharge as new generation would double-count energy already tallied when it was first produced. Because each grid operator reports batteries differently (some net, some discharge-only), storage is sourced uniformly from EIA-930 to keep them comparable. The big battery markets read correct net (CAISO/ERCOT ~−6 to −8 GWh/day, MISO slightly negative); a few small markets (PJM, SPP, ISO-NE) still read small-positive because those operators only file discharge with EIA — all under 0.1% of their generation. Pumped hydro is counted as hydro, not storage. Note that battery reporting conventions have changed over time at the source (e.g. ERCOT's EIA history is discharge-only until it switched to net reporting in late 2025), so a storage line can show a one-time level shift in older data; recent data is consistent. Storage is under 1% of generation and doesn't affect the gas/coal/nuclear picture.
